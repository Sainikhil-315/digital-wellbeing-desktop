...
function getTodayUsage() {
  return query(`
    SELECT ul.app_name, SUM(ul.duration_seconds) as total_seconds,
      COALESCE(ka.category, 'Other') AS category
    FROM usage_log ul
    LEFT JOIN known_apps ka ON LOWER(ka.app_name) = LOWER(ul.app_name)
    WHERE ul.date = ? AND ${buildExcludeClause('ul.app_name')}
    GROUP BY ul.app_name
...
function getWeeklyTopApps() {
  const days = buildLocalDays(7)
  return query(`
    SELECT ul.app_name, SUM(ul.duration_seconds) as total_seconds, COUNT(DISTINCT ul.date) as days_active,
      COALESCE(ka.category, 'Other') AS category
    FROM usage_log ul
    LEFT JOIN known_apps ka ON LOWER(ka.app_name) = LOWER(ul.app_name)
    WHERE ul.date >= ? AND ${buildExcludeClause('ul.app_name')}
...
const PRODUCTIVE_CATEGORIES   = new Set(['development', 'productivity'])
const DISTRACTING_CATEGORIES  = new Set(['entertainment', 'gaming', 'social'])

function getEarliestUsageDate() {
  const rows = query(`SELECT MIN(date) as d FROM usage_log`)
  return rows[0]?.d || today()
}

function rangeStart(rangeKey) {
  if (rangeKey === 'all') return getEarliestUsageDate()
  const n = RANGE_DAYS[rangeKey] || 30
  const d = new Date()
  d.setDate(d.getDate() - (n - 1))
  return d.toISOString().slice(0, 10)
}

// Consolidated data set for the Reports screen — totals, trend, rankings,
// category split, comparison vs the immediately preceding period of equal
// length, and a "typical week" hour x weekday heatmap averaged across the range.
function getReportData(rangeKey = '30d') {
  const startDate = rangeStart(rangeKey)
  const endDate = today()
  ...
  `, [startDate, endDate])
  const dailyMap = {}
  for (const r of dailyRows) dailyMap[r.date] = Number(r.total_seconds)
  ...
  while (cur <= end) {
    ...
    daily.push({ date: dateStr, total_seconds: dailyMap[dateStr] || 0 })
    cur.setDate(cur.getDate() + 1)
  }

  const totalSeconds = daily.reduce((s, d) => s + d.total_seconds, 0)
  const activeDays = daily.filter(d => d.total_seconds > 0).length
  const avgSeconds = activeDays ? Math.round(totalSeconds / activeDays) : 0
  const withData = daily.filter(d => d.total_seconds > 0)
  const peakDay = withData.length ? withData.reduce((a, d) => d.total_seconds > a.total_seconds ? d : a) : null
  const bestDay = withData.length ? withData.reduce((a, d) => d.total_seconds < a.total_seconds ? d : a) : null

  // Longest consecutive-day streak with usage inside this range
  let longestStreak = 0, run_ = 0
  for (const d of daily) {
    if (d.total_seconds > 0) { run_++; longestStreak = Math.max(longestStreak, run_) }
    else run_ = 0
  }
  ...
  const prevRows = query(`
    SELECT SUM(duration_seconds) as total FROM usage_log
    WHERE date >= ? AND date <= ? AND ${buildExcludeClause('app_name')}
  `, [prevStart.toISOString().slice(0, 10), prevEnd.toISOString().slice(0, 10)])
  const prevTotalSeconds = Number(prevRows[0]?.total || 0)
  ...
  const topApps = query(`
    ...
    FROM usage_log ul
    LEFT JOIN known_apps ka ON LOWER(ka.app_name) = LOWER(ul.app_name)
    WHERE ul.date >= ? AND ul.date <= ? AND ${buildExcludeClause('ul.app_name')}
    GROUP BY ul.app_name
    ORDER BY total_seconds DESC
    LIMIT 15
  `, [startDate, endDate])
  ...
  const categoryBreakdown = query(`
    SELECT COALESCE(ka.category, 'Other') AS category, SUM(ul.duration_seconds) AS total_seconds
    FROM usage_log ul
    LEFT JOIN known_apps ka ON LOWER(ul.app_name) = LOWER(ka.app_name)
    WHERE ul.date >= ? AND ul.date <= ? AND ${buildExcludeClause('ul.app_name')}
    GROUP BY COALESCE(ka.category, 'Other')
    ORDER BY total_seconds DESC
  `, [startDate, endDate])

  // Per-day productive vs distracting vs neutral split, for the balance trend chart
  const dailyCatRows = query(`
    SELECT ul.date, COALESCE(ka.category, 'Other') AS category, SUM(ul.duration_seconds) AS total_seconds
    FROM usage_log ul
    LEFT JOIN known_apps ka ON LOWER(ul.app_name) = LOWER(ka.app_name)
    WHERE ul.date >= ? AND ul.date <= ? AND ${buildExcludeClause('ul.app_name')}
    GROUP BY ul.date, COALESCE(ka.category, 'Other')
  `, [startDate, endDate])
  const dailyGroupMap = {}
  for (const row of dailyCatRows) {
    const cat = (row.category || 'Other').toLowerCase()
    const bucket = PRODUCTIVE_CATEGORIES.has(cat) ? 'productive'
      : DISTRACTING_CATEGORIES.has(cat) ? 'distracting'
      : 'neutral'
    if (!dailyGroupMap[row.date]) dailyGroupMap[row.date] = { productive: 0, distracting: 0, neutral: 0 }
    dailyGroupMap[row.date][bucket] += Number(row.total_seconds)
  }
  const dailyGroups = daily.map(d => ({
    date: d.date,
    productive_seconds: dailyGroupMap[d.date]?.productive || 0,
    distracting_seconds: dailyGroupMap[d.date]?.distracting || 0,
    neutral_seconds: dailyGroupMap[d.date]?.neutral || 0,
  }))
  ...
  return {
    ...
    prev_total_seconds: prevTotalSeconds,
    top_apps: topApps.map(a => ({ ...a, total_seconds: Number(a.total_seconds), days_active: Number(a.days_active) })),
    category_breakdown: categoryBreakdown.map(c => ({ ...c, total_seconds: Number(c.total_seconds) })),
    weekday_hour_avg: avgMatrix,
